...
// appCSS — shared stylesheet, borrowed from sub2api's design language
// (teal primary #14b8a6, slate grays, gradient bg + blurred orbs + faint grid,
// rounded-xl cards with shadow, soft-tint badges).
const appCSS = `
:root{
...
body::before{content:"";position:fixed;inset:0;pointer-events:none;z-index:0;
  background:
   radial-gradient(55rem 55rem at 108% -8%,rgba(20,184,166,.10),transparent 60%),
   radial-gradient(45rem 45rem at -8% 108%,rgba(20,184,166,.07),transparent 60%),
   linear-gradient(rgba(20,184,166,.025) 1px,transparent 1px) 0 0/64px 64px,
   linear-gradient(90deg,rgba(20,184,166,.025) 1px,transparent 1px) 0 0/64px 64px}
header.top,main,footer{position:relative;z-index:1}
a{color:var(--primary-700);text-decoration:none}
a:hover{text-decoration:underline}
header.top{position:sticky;top:0;z-index:20;background:rgba(255,255,255,.82);backdrop-filter:blur(10px);border-bottom:1px solid var(--border);padding:.55rem 1.2rem}
...
thead th{position:sticky;top:0;background:#f1f5f8;text-align:left;padding:.55rem .7rem;border-bottom:1px solid var(--border);white-space:nowrap;font-weight:600;color:#334155}
...
func pageShell(title, active, body string) string {
	...
	for _, it := range navItems {
		...
		fmt.Fprintf(&n, `<a href="%s"%s>%s</a>`, it.H, cls, it.Label)
	}
	return fmt.Sprintf(`<!doctype html><html lang="zh"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>%s · TransitMonitor</title><style>%s</style></head>`+
		`<body><header class="top"><div class="top-row"><div class="brand"><span class="logo">TM</span>TransitMonitor</div><nav>%s</nav></div></header>`+
		`<main>%s</main><footer>TransitMonitor · <a href="/api/stations">JSON API</a> · <a href="/metrics">/metrics</a> (Prometheus) · <a href="/healthz">/healthz</a></footer></body></html>`,
		html.EscapeString(title), appCSS, n.String(), body)
}

func writeHTMLShell(w http.ResponseWriter, title, active, body string) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	_, _ = w.Write([]byte(pageShell(title, active, body)))
}

// renderTable renders a styled table. Cells are raw HTML (callers escape plain text;
// use badge helpers for status/severity).
