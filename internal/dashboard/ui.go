...
func pageShell(lang, title, active, body string, showLogout bool) string {
	...
	js := `<script>(function(){var d=document.documentElement,s=localStorage.getItem('tm-theme');` +
		...
		`if(ar==='1'){var tmDirty=false;window.tmMarkDirty=function(){tmDirty=true;};document.addEventListener('input',function(){tmDirty=true;},true);document.addEventListener('change',function(){tmDirty=true;},true);/* skip auto-reload once the user has edited a field (incl. drag-reorder), so unsaved edits aren't wiped */setTimeout(function(){if(!tmDirty){location.reload();}},60000);}` +
		...
		`});document.addEventListener('click',function(e){document.querySelectorAll('.csel.open').forEach(function(c){c.classList.remove('open')});document.querySelectorAll('.st-sel.open').forEach(function(c){if(!c.contains(e.target))c.classList.remove('open')})});` +
